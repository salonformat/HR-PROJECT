...
function applyLanguage(nextLanguage, rerender = true) {
  ...
  $(".adaptable-note strong").textContent = d.adaptable; $(".adaptable-note p").textContent = d.adaptableText; $(".adaptable-note").setAttribute("aria-label", d.adaptable);
...
function updateHeader() {
  ...
  $("#global-progress-fill").style.width = `${completed.length / 15 * 100}%`;
}
...
function renderMission() {
  ...
  $("#mission-location").textContent = `${mc.number} ${Number(mission.number)} ${mc.of} · ${phase.label}`;
  ...
  $("#focused-mission").innerHTML = `<header><div class="mission-meta"><span>${mc.number} ${Number(mission.number)} ${mc.of}</span><span>${phase.label}</span><span>${durationLabel(mission.duration)}</span></div><span class="mission-big-number" aria-hidden="true">${Number(mission.number)}</span><h1>${mission.title}</h1><p>${mission.intro}</p></header><section class="focused-steps"><h2>${mc.steps}</h2><ol>${mission.steps.map((step) => `<li>${step}</li>`).join("")}</ol></section>${support}<form id="mission-form"><div class="mission-form-title"><h2>${mc.results}</h2><p>${mc.savedLocal}</p></div>${mission.prompts.map((prompt, index) => `<label><span>${prompt[0]}${prompt[2] === false ? ` <small>${mc.optional}</small>` : ""}</span><textarea rows="4" data-answer="${index}" ${prompt[2] === false ? "" : "required"} placeholder="${prompt[1]}">${escapeHtml(saved[index] || "")}</textarea></label>`).join("")}<p class="form-status" role="status"></p><div class="form-actions"><button type="button" data-save-mission>${mc.save}</button><button type="submit" class="primary-action">${isDone ? mc.update : mc.finish}</button></div></form>${completion}`;
  updateHeader();
}
